...
import { TradingRewardExpiration } from "./module/types/rewards/trading_reward"
import { TradingRewardLeaderboard } from "./module/types/rewards/trading_reward"
import { TradingRewardLeaderboardEntry } from "./module/types/rewards/trading_reward"
import { TradingRewardCandidate } from "./module/types/rewards/trading_reward"
	...
	actions: {
		...
		async QueryTradingRewardAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryTradingRewardAll( key.state, query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryTradingRewardAll( key.state, {...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
		...
		async QueryGetTradingRewardLeaderboardHandler({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryGetTradingRewardLeaderboardHandler( key.reward_id)).data
		...
		async QueryGetMarketIdTradingRewardIdHandler({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryGetMarketIdTradingRewardIdHandler(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryGetMarketIdTradingRewardIdHandler({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
		...
		async QueryAllPendingUnlockParticipant({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryAllPendingUnlockParticipant(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryAllPendingUnlockParticipant({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
