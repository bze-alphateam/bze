...
const baseBurnCoinsProposal: object = { title: "", description: "" };

export const BurnCoinsProposal = {
  encode(message: BurnCoinsProposal, writer: Writer = Writer.create()): Writer {
    if (message.title !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): BurnCoinsProposal {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseBurnCoinsProposal } as BurnCoinsProposal;
