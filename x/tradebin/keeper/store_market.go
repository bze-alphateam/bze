...
func (k Keeper) getMarketStore(ctx sdk.Context) prefix.Store {
	return k.getPrefixedStore(ctx, types.KeyPrefix(types.MarketKeyPrefix))
}

func (k Keeper) getMarketAliasStore(ctx sdk.Context) prefix.Store {
	return k.getPrefixedStore(ctx, types.KeyPrefix(types.MarketAliasKeyPrefix))
}
...
func (k Keeper) GetAllAssetMarkets(ctx sdk.Context, asset string) (list []types.Market) {
	store := k.getMarketStore(ctx)
	iterator := storetypes.KVStorePrefixIterator(store, types.MarketAssetKey(asset))
...
func (k Keeper) MarketExists(ctx sdk.Context, marketId string) bool {
	...
	_, ok = k.GetLiquidityPool(ctx, marketId)
