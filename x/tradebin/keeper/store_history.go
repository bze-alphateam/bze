...
func (k Keeper) getHistoryOrderStore(ctx sdk.Context) prefix.Store {
	return k.getPrefixedStore(ctx, types.KeyPrefix(types.HistoryOrderKeyPrefix))
}

func (k Keeper) getHistoryOrderByMarketStore(ctx sdk.Context, marketId string) prefix.Store {
	return k.getPrefixedStore(ctx, types.HistoryOrderByMarketPrefix(marketId))
}
