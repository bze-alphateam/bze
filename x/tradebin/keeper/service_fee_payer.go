...
func (k Keeper) CaptureAndSwapUserFee(ctx sdk.Context, payer sdk.AccAddress, fee sdk.Coins) (coins sdk.Coins, err error) {
	...
	if ctxDenom == "" || nativeDenom == ctxDenom {
		...
		return k.payerCoinsToModule(ctx, payer, fee)
	}
	...
	if payerBalances.IsAllGTE(toCapture) {
		...
		_, err = k.payerCoinsToModule(ctx, payer, toCapture)
		if err != nil {
			return nil, err
		}
...
func (k Keeper) payerCoinsToModule(ctx sdk.Context, payer sdk.AccAddress, coins sdk.Coins) (sdk.Coins, error) {
	err := k.bankKeeper.SendCoinsFromAccountToModule(ctx, payer, types.ModuleName, coins)
	if err != nil {
		return nil, err
	}
