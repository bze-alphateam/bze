...
func networkWithMarketObjects(t *testing.T, n int) (*network.Network, []types.Market) {
	...
	for i := 0; i < n; i++ {
		market := types.Market{
			Asset1: strconv.Itoa(i),
			Asset2: strconv.Itoa(i),
		}
		nullify.Fill(&market)
...
func TestShowMarket(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:     "found",
			idAsset1: objs[0].Asset1,
			idAsset2: objs[0].Asset2,
		...
		{
			desc:     "not found",
			idAsset1: strconv.Itoa(100000),
			idAsset2: strconv.Itoa(100000),
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetMarketResponse
...
func TestListMarket(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.NoError(t, err)
			var resp types.QueryAllMarketResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
