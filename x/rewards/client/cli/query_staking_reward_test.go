...
func networkWithStakingRewardObjects(t *testing.T, n int) (*network.Network, []types.StakingReward) {
	t.Helper()
	...
	return network.New(t, cfg), state.StakingRewardList
}

func TestShowStakingReward(t *testing.T) {
	net, objs := networkWithStakingRewardObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetStakingRewardResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.StakingReward)
				require.Equal(t,
					nullify.Fill(&tc.obj),
...
func TestListStakingReward(t *testing.T) {
	net, objs := networkWithStakingRewardObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListStakingReward(), args)
			require.NoError(t, err)
			var resp types.QueryAllStakingRewardResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
