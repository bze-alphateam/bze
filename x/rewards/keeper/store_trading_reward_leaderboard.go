...
func (k Keeper) SetTradingRewardLeaderboard(ctx sdk.Context, leaderboard types.TradingRewardLeaderboard) {
	store := k.getPrefixedStore(ctx, types.KeyPrefix(types.LeaderboardKeyPrefix))
...
func (k Keeper) SetTradingRewardCandidate(ctx sdk.Context, entry types.TradingRewardCandidate) {
	store := k.getPrefixedStore(ctx, types.KeyPrefix(types.RewardCandidateKeyPrefix))
...
func (k Keeper) GetTradingRewardCandidateByRewardId(ctx sdk.Context, rewardId string) (list []types.TradingRewardCandidate) {
	store := k.getPrefixedStore(ctx, types.KeyPrefix(types.RewardCandidateKeyPrefix))
	iterator := storetypes.KVStorePrefixIterator(store, []byte(rewardId+"/"))
