...
# Filtering by containers belonging to the "bze-testnet" network
LIST_CONTAINERS_CMD=$(docker ps -a --filter network=bze-testnet --format {{.ID}})
LIST_NETWORKS_CMD=$(docker network ls --filter name=bze-testnet --format {{.ID}})
...
for cur_image_sha in $LIST_DOCKER_IMAGE_HASHES; do
    if [[ "$cur_image_sha" != "$local_git_sha" ]]; then
        ...
        docker rmi -f $(docker images --filter=reference="bze:$cur_image_sha" -q)
    fi
done
