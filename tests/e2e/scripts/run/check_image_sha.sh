...
check_if_exists() {
    if [[ "$(docker images -q bze 2> /dev/null)" != "" ]]; then
        return 1
    fi
    return 0
}
...
if [[ "$exists" -eq 1 ]]; then 
    echo "bze:debug image found"
    
    check_if_up_to_date
    up_to_date=$?

    if [[ "$up_to_date" -eq 1 ]]; then
        echo "bze:debug image is up to date; nothing is done"
        exit 0
    else
        echo "bze:debug image is not up to date; rebuilding"
    fi
else
    echo "bze:debug image not found; building"
fi
