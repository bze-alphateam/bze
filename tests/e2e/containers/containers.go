...
var (
	//.0025 ubze
	ConsensusMinFee = sdkmath.LegacyNewDecWithPrec(25, 4)

	//.0025 ubze/gas
	Fees = ConsensusMinFee.Mul(sdkmath.LegacyNewDec(GasLimit)).Ceil().TruncateInt64()
	...
	txDefaultGasArgs = []string{fmt.Sprintf("--gas=%d", GasLimit), fmt.Sprintf("--fees=%d%s", Fees, initialization.E2EFeeToken)}
)
...
func NewManager(isUpgrade bool, isFork bool, isDebugLogEnabled bool) (docker *Manager, err error) {
	...
	docker.network, err = docker.pool.CreateNetwork("bze-testnet")
	if err != nil {
		return nil, err
	}
	return docker, nil
...
func (m *Manager) RunHermesResource(chainAID, osmoARelayerNodeName, osmoAValMnemonic, chainBID, osmoBRelayerNodeName, osmoBValMnemonic string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("BZE_A_E2E_CHAIN_ID=%s", chainAID),
				fmt.Sprintf("BZE_B_E2E_CHAIN_ID=%s", chainBID),
				fmt.Sprintf("BZE_A_E2E_VAL_MNEMONIC=%s", osmoAValMnemonic),
				fmt.Sprintf("BZE_B_E2E_VAL_MNEMONIC=%s", osmoBValMnemonic),
				fmt.Sprintf("BZE_A_E2E_VAL_HOST=%s", osmoARelayerNodeName),
				fmt.Sprintf("BZE_B_E2E_VAL_HOST=%s", osmoBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",
...
func (m *Manager) RunNodeResource(chainId string, containerName, valCondifDir string) (*dockertest.Resource, error) {
	...
	runOpts := &dockertest.RunOptions{
		Name:       containerName,
		Repository: m.BzeRepository,
		Tag:        m.BzeTag,
		NetworkID:  m.network.Network.ID,
		User:       "root:root",
		Cmd:        []string{"start"},
		Mounts: []string{
			fmt.Sprintf("%s/:/bze/.bzed", valCondifDir),
			fmt.Sprintf("%s/scripts:/bze", pwd),
		},
	}
