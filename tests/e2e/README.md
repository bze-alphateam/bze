...
depending on the bze codebase. It initializes the chains for testing
via Docker files. As a result, the test suite may provide the desired
bze version to Docker containers during the initialization. This
design allows for the opportunity of testing chain upgrades in the
future by providing an older bze version to the container,
...
1. Chain Initialization
    ...
    of the previous Bze version, inside `configurer/upgrade.go`. This is
    ...
    `UpgradeConfigurer` takes older bze versions and configures chains from them. 
    ...
    current branch version of Bze codebase
    ...
    with IBC initialized at the previous Bze version
...
2. Setting up e2e components
    ...
    - IBC testing
        ...
        - Upgrade Testing
        - 2 chains of the older Bze version are created, and
    ...
    - Upgrade testing
        ...
        - Current branch Bze version is spun up to continue with testing
...
This package directly depends on the Bze codebase.
...
production osmosis container to start the `bzed` service.
...
- `BZESIS_E2E_SKIP_UPGRADE` - when true, skips the upgrade tests.
If BZESIS_E2E_SKIP_IBC is true, this must also be set to true because upgrade
tests require IBC logic.

- `BZESIS_E2E_SKIP_IBC` - when true, skips the IBC tests.

- `BZESIS_E2E_SKIP_STATE_SYNC` - when true, skips the state sync tests.

- `BZESIS_E2E_SKIP_CLEANUP` - when true, avoids cleaning up the e2e Docker
containers.

- `BZESIS_E2E_FORK_HEIGHT` - when the above "IS_FORK" env variable is set to true, this is the string
of the height in which the network should fork. This should match the ForkHeight set in constants.go

- `BZESIS_E2E_UPGRADE_VERSION` - string of what version will be upgraded to (for example, "v10")

- `BZESIS_E2E_DEBUG_LOG` - when true, prints debug logs from executing CLI commands
    ...
    "configurations": [
        {
            ...
            "env": {
                "BZE_E2E": "True",
                "BZE_E2E_SKIP_IBC": "true",
                "BZE_E2E_SKIP_UPGRADE": "true",
                "BZE_E2E_SKIP_CLEANUP": "true",
                "BZE_E2E_SKIP_STATE_SYNC": "true",
                "BZE_E2E_UPGRADE_VERSION": "v13",
                "BZE_E2E_DEBUG_LOG": "true",
            },
...
1. Order of executions of tests in `e2e_test.go` is alphabetic.
    ...
    A way to deal with this problem: disable upgrade logic by setting `BZE_E2E_SKIP_UPGRADE` to `false` and see how test performs.
...
We set the following parameters Hermes configs to enable the consensus min fee in Bze:
    ...
    In Bze, we set consensus min fee = .0025 uosmo / gas * 400000 gas = 1000
...
1. Executing commands on docker containers
    ...
    Example: `docker exec osmo-test-a-node-prune-nothing-snapshot bzed status` will print a node status of `osmo-test-a-node-prune-nothing-snapshot` container. 
