...
import (
	...
	"errors"
	"fmt"
	params2 "github.com/bze-alphateam/bze/app/params"
	"os"
	"regexp"
	"strconv"
	"strings"
...
func (n *NodeConfig) SetMaxPoolPointsPerTx(maxPoolPoints int, from string) {
	...
	cmd := []string{"bzed", "tx", "protorev", "set-max-pool-points-per-tx", fmt.Sprintf("%d", maxPoolPoints), fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000uosmo"}
...
func (n *NodeConfig) CreateBalancerPool(poolFile, from string) uint64 {
	...
	cmd := []string{"bzed", "tx", "gamm", "create-pool", fmt.Sprintf("--pool-file=/bze/%s", poolFile), fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000uosmo"}
...
func (n *NodeConfig) CreateStableswapPool(poolFile, from string) uint64 {
	...
	cmd := []string{"bzed", "tx", "gamm", "create-pool", fmt.Sprintf("--pool-file=/bze/%s", poolFile), "--pool-type=stableswap", fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000uosmo"}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CollectSpreadRewards(from, positionIds string) {
	...
	cmd := []string{"bzed", "tx", "concentratedliquidity", "collect-spread-rewards", positionIds, fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateConcentratedPool(from, denom1, denom2 string, tickSpacing uint64, spreadFactor string) uint64 {
	...
	cmd := []string{"bzed", "tx", "concentratedliquidity", "create-pool", denom1, denom2, fmt.Sprintf("%d", tickSpacing), spreadFactor, fmt.Sprintf("--from=%s", from)}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateConcentratedPosition(from, lowerTick, upperTick string, tokens string, token0MinAmt, token1MinAmt int64, poolId uint64) (uint64, sdkmath.LegacyDec) {
	...
	cmd := []string{"bzed", "tx", "concentratedliquidity", "create-position", fmt.Sprint(poolId), lowerTick, upperTick, tokens, fmt.Sprintf("%d", token0MinAmt), fmt.Sprintf("%d", token1MinAmt), fmt.Sprintf("--from=%s", from), "--gas=500000", "--fees=1250uosmo", "-o json"}
...
func (n *NodeConfig) StoreWasmCode(wasmFile, from string) int {
	...
	cmd := []string{"bzed", "tx", "wasm", "store", wasmFile, fmt.Sprintf("--from=%s", from), "--gas=auto", "--gas-prices=0.1uosmo", "--gas-adjustment=1.3"}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) WithdrawPosition(from, liquidityOut string, positionId uint64) {
	...
	cmd := []string{"bzed", "tx", "concentratedliquidity", "withdraw-position", fmt.Sprint(positionId), liquidityOut, fmt.Sprintf("--from=%s", from), "--gas=700000", "--fees=5000uosmo"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) InstantiateWasmContract(codeId, initMsg, from string) {
	...
	cmd := []string{"bzed", "tx", "wasm", "instantiate", codeId, initMsg, fmt.Sprintf("--from=%s", from), "--no-admin", "--label=contract"}
...
func (n *NodeConfig) WasmExecute(contract, execMsg, from string) {
	...
	cmd := []string{"bzed", "tx", "wasm", "execute", contract, execMsg, fmt.Sprintf("--from=%s", from)}
	n.LogActionF(strings.Join(cmd, " "))
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryParams(subspace, key string) string {
	cmd := []string{"bzed", "query", "params", "subspace", subspace, key, "--output=json"}
...
func (n *NodeConfig) SubmitParamChangeProposal(proposalJson, from string) int {
	...
	cmd := []string{"bzed", "tx", "gov", "submit-proposal", "param-change", fmt.Sprintf("/osmosis/param_change_proposal_%s.json", currentTime), fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) SendIBCTransfer(dstChain *Config, from, recipient, memo string, token sdk.Coin) {
	...
	cmd := []string{"bzed", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, token.String(), fmt.Sprintf("--from=%s", from), "--memo", memo}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) FailIBCTransfer(from, recipient, amount string) {
	...
	cmd := []string{"bzed", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, amount, fmt.Sprintf("--from=%s", from)}
...
// docker container exec <container id> bzed tx gamm swap-exact-amount-in <tokeinInCoin> <tokenOutMinAmountInt> --swap-route-pool-ids <swapRoutePoolIds> --swap-route-denoms <swapRouteDenoms> --chain-id=<id>--from=<address> --keyring-backend=test -b=block --yes --log_format=json
func (n *NodeConfig) SwapExactAmountIn(tokenInCoin, tokenOutMinAmountInt string, swapRoutePoolIds string, swapRouteDenoms string, from string) {
	...
	cmd := []string{"bzed", "tx", "gamm", "swap-exact-amount-in", tokenInCoin, tokenOutMinAmountInt, fmt.Sprintf("--swap-route-pool-ids=%s", swapRoutePoolIds), fmt.Sprintf("--swap-route-denoms=%s", swapRouteDenoms), fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) JoinPoolExactAmountIn(tokenIn string, poolId uint64, shareOutMinAmount string, from string) {
	...
	cmd := []string{"bzed", "tx", "gamm", "join-swap-extern-amount-in", tokenIn, shareOutMinAmount, fmt.Sprintf("--pool-id=%d", poolId), fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) ExitPool(from, minAmountsOut string, poolId uint64, shareAmountIn string) {
	...
	cmd := []string{"bzed", "tx", "gamm", "exit-pool", fmt.Sprintf("--min-amounts-out=%s", minAmountsOut), fmt.Sprintf("--share-amount-in=%s", shareAmountIn), fmt.Sprintf("--pool-id=%d", poolId), fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) SubmitUpgradeProposal(upgradeVersion string, upgradeHeight int64, initialDeposit sdk.Coin) int {
	...
	cmd := []string{"bzed", "tx", "gov", "submit-proposal", "software-upgrade", upgradeVersion, fmt.Sprintf("--title=\"%s upgrade\"", upgradeVersion), "--description=\"upgrade proposal submission\"", fmt.Sprintf("--upgrade-height=%d", upgradeHeight), "--upgrade-info=\"\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) SubmitSuperfluidProposal(asset string, initialDeposit sdk.Coin) int {
	...
	cmd := []string{"bzed", "tx", "gov", "submit-proposal", "set-superfluid-assets-proposal", fmt.Sprintf("--superfluid-assets=%s", asset), "--title=\"superfluid asset prop\"", fmt.Sprintf("--description=\"%s superfluid asset\"", asset), "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit), "--gas=700000", "--fees=5000uosmo"}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) SubmitCreateConcentratedPoolProposal(initialDeposit sdk.Coin) int {
	...
	cmd := []string{"bzed", "tx", "gov", "submit-proposal", "create-concentratedliquidity-pool-proposal", "--pool-records=stake,uosmo,100,0.001", "--title=\"create concentrated pool\"", "--description=\"create concentrated pool", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) SubmitTextProposal(text string, initialDeposit sdk.Coin, isExpedited bool) int {
	...
	cmd := []string{"bzed", "tx", "gov", "submit-proposal", "--type=text", fmt.Sprintf("--title=\"%s\"", text), "--description=\"test text proposal\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	if isExpedited {
...
func (n *NodeConfig) SubmitTickSpacingReductionProposal(poolTickSpacingRecords string, initialDeposit sdk.Coin, isExpedited bool) int {
	...
	cmd := []string{"bzed", "tx", "gov", "submit-proposal", "tick-spacing-decrease-proposal", "--title=\"test tick spacing reduction proposal title\"", "--description=\"test tick spacing reduction proposal\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit), fmt.Sprintf("--pool-tick-spacing-records=%s", poolTickSpacingRecords)}
	if isExpedited {
		cmd = append(cmd, "--is-expedited=true")
	}
	resp, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
...
func (n *NodeConfig) DepositProposal(proposalNumber int, isExpedited bool) {
	...
	deposit := sdk.NewCoin(initialization.BzeDenom, sdkmath.NewInt(config.MinDepositValue)).String()
	if isExpedited {
		deposit = sdk.NewCoin(initialization.BzeDenom, sdkmath.NewInt(config.MinExpeditedDepositValue)).String()
	}
	cmd := []string{"bzed", "tx", "gov", "deposit", fmt.Sprintf("%d", proposalNumber), deposit, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteYesProposal(from string, proposalNumber int) {
	...
	cmd := []string{"bzed", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "yes", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteNoProposal(from string, proposalNumber int) {
	...
	cmd := []string{"bzed", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "no", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BankSend(amount string, sendAddress string, receiveAddress string) {
	...
	cmd := []string{"bzed", "tx", "bank", "send", sendAddress, receiveAddress, amount, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) FundCommunityPool(sendAddress string, funds string) {
	...
	cmd := []string{"bzed", "tx", "distribution", "fund-community-pool", funds, fmt.Sprintf("--from=%s", sendAddress), "--gas=600000", "--fees=1500uosmo"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateWallet(walletName string, chain *Config) string {
	...
	cmd := []string{"bzed", "keys", "add", walletName, "--keyring-backend=test"}
...
func (n *NodeConfig) AddExistingWallet(walletName, mnemonic string) {
	...
	cmd := []string{"sh", "-c", fmt.Sprintf("echo '%s' | bzed keys add %s --keyring-backend=test --recover", mnemonic, walletName)}
...
func (n *NodeConfig) GetWallet(walletName string) string {
	...
	cmd := []string{"bzed", "keys", "show", walletName, "--keyring-backend=test"}
...
func (n *NodeConfig) QueryPropStatusTimed(proposalNumber int, desiredStatus string, totalTime chan time.Duration) {
	...
	require.Eventually(
		...
		1*time.Minute,
		10*time.Millisecond,
		"Bze node failed to retrieve prop tally",
	)
...
func (n *NodeConfig) EnableSuperfluidAsset(srcChain *Config, denom string) {
	propNumber := n.SubmitSuperfluidProposal(denom, sdk.NewCoin(initialization.BzeDenom, sdkmath.NewInt(config.InitialMinDeposit)))
...
func (n *NodeConfig) ParamChangeProposal(subspace, key string, value []byte, chain *Config) error {
	proposal := paramsutils.ParamChangeProposalJSON{
		...
		Deposit: fmt.Sprintf("625000000uosmo%s", initialization.BzeDenom),
	}
...
func extractLiquidityFromResponse(responseBytes []byte) (sdkmath.LegacyDec, error) {
	var txResponse map[string]interface{}
	err := json.Unmarshal(responseBytes, &txResponse)
	if err != nil {
	...
	positionID, err := sdkmath.LegacyNewDecFromStr(liquidityString)
	if err != nil {
		return sdkmath.LegacyDec{}, err
	}
