...
func (c *Config) SendIBC(dstChain *Config, recipient string, token sdk.Coin) {
	...
	removeFeeTokenFromBalance := func(balance sdk.Coins) sdk.Coins {
		var foundCoins []sdk.Coin
		for _, coin := range balance {
			if !strings.HasPrefix(coin.Denom, "ibc/") {
				ok, findResult := balance.Find(coin.Denom)
				if !ok {
					continue
				}

				foundCoins = append(foundCoins, findResult)
			}
		}

		return balance.Sub(foundCoins...)
	}
...
func (c *Config) SubmitCreateConcentratedPoolProposal(chainANode *NodeConfig) (uint64, error) {
	propNumber := chainANode.SubmitCreateConcentratedPoolProposal(sdk.NewCoin(initialization.BzeDenom, sdkmath.NewInt(config.InitialMinDeposit)))
