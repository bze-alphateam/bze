...
* (x/tradebin) [#115](https://github.com/bze-alphateam/bze/pull/115) Governance-halted denoms: with a denom listed in the new `halted_denoms` parameter, the EndBlock engine refunds every queued buy / sell / fill message on a market whose base or quote is that denom instead of matching it (typed `QueueMessageRefundedEvent`, reason `market_halted`; cancels are processed unchanged), `swapTokens` refuses any pool holding the denom, and the liquidity answers the ante handler, fee collector and burner rely on (`HasLiquidityWithNativeDenom`, `HasDeepLiquidityWithNativeDenom`, `CanSwapForNativeDenom`) are false for it. The list ships **empty**, so behaviour is unchanged until a governance proposal lists a denom; no migration, tradebin consensus version stays 4. No action for validators.
