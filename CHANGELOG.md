...
The handler also carries the Noble USDC wind-down. Circle is retiring USDC on the Noble chain, so from 2027-01-12 the `USDC.n` vouchers held on BeeZee lose their backing. At the upgrade height the chain stops accepting new inbound `uusdc` from Noble (exits stay open — outgoing transfers, refunds and the channel itself are untouched) and **transfers the protocol-owned USDC.n/BZE liquidity-pool shares held by the burner black hole to the team's admin address `bze1jx4x3kn8mlz2s03zdpf2a38x9gl66llvjqdd55`**, so that liquidity can be withdrawn, migrated to Injective-issued USDC (`USDC.inj`) and locked in the black hole again as a USDC.inj/BZE pool. Every other black-hole balance stays where it is. The migration values are hardcoded for this release and will be removed afterwards.

### State Machine Breaking

* (x/tradebin) [#115](https://github.com/bze-alphateam/bze/pull/115) Governance-halted denoms: with a denom listed in the new `halted_denoms` parameter, `swapTokens` refuses any pool holding the denom (no user route, module swap or fee swap can exchange it) and the liquidity answers the ante handler, fee collector and burner rely on (`HasLiquidityWithNativeDenom`, `HasDeepLiquidityWithNativeDenom`, `CanSwapForNativeDenom`) are false for it. The EndBlock order book engine is untouched: messages already queued when a halt takes effect execute as always. The list ships **empty**, so behaviour is unchanged until a governance proposal lists a denom; no migration, tradebin consensus version stays 4. No action for validators.
* (x/txfeecollector) [#114](https://github.com/bze-alphateam/bze/pull/114) New `blocked_ibc_inbound` parameter (list of `{channel_id, base_denom}` pairs, empty by default) and the IBC middleware that reads it: an incoming ICS-20 packet matching a listed pair is answered with an error acknowledgement instead of minting a voucher, and the sending chain refunds the sender when it relays that acknowledgement. Only received packets are inspected — outgoing transfers, their acknowledgements, their timeout refunds and the channel lifecycle are unchanged, and vouchers of tokens that originally left BeeZee always pass. Every refused packet emits a `BlockedIbcInboundEvent` typed event (see `proto/bze/txfeecollector/events.proto`). txfeecollector consensus version 2→3; the migration leaves the list empty, governance edits it through `MsgUpdateParams`.
* (app) [#114](https://github.com/bze-alphateam/bze/pull/114) Noble USDC wind-down in the `v8.2.0` upgrade handler: `blocked_ibc_inbound` is set to `{channel-3, uusdc}`, so new Noble USDC can no longer enter the chain, and the burner black hole's entire balance of the USDC.n/BZE pool share denom (`ulp_ibc/6490A7EA…_ubze`) is transferred to `bze1jx4x3kn8mlz2s03zdpf2a38x9gl66llvjqdd55` — protocol-owned liquidity handed to the admin address so it can be redeployed as USDC.inj liquidity and locked again. Every other black-hole balance is untouched; a zero balance is a logged no-op, and a failed transfer is logged instead of failing the upgrade. The testnet (`bzetestnet-3`) rehearses only the share transfer, against a pool its own black hole holds; it blocks no inbound transfer.
...
* (x/tradebin) [#115](https://github.com/bze-alphateam/bze/pull/115) New `halted_denoms` tradebin parameter (repeated string, empty by default, at most 32 exact-match denoms, never the native denom), set and cleared through a `MsgUpdateParams` governance proposal. A market or pool whose base or quote is halted refuses `MsgCreateMarket`, `MsgCreateLiquidityPool`, `MsgCreateOrder`, `MsgFillOrders`, `MsgAddLiquidity` and any `MsgMultiSwap` route touching it (also as an intermediate hop) with the new `ErrDenomHalted` (4017), before any fee is captured or funds escrowed; messages already queued when the halt takes effect (accepted a block or a few earlier) still execute normally, so the queue simply drains. Exits always work: `MsgCancelOrder`, `MsgRemoveLiquidity`, bank sends, IBC transfers; resting orders and pool reserves are left untouched. A halted denom is never swapped by anyone — users, modules or fee swaps — so it stops being accepted as a tx fee denom and any fee dust already collected in it follows the existing non-swappable path (fee collector → burner → black hole); tx-fee dust in it goes to stakers in kind. Un-halting is the same proposal without the denom. Runbook in `x/tradebin/docs/README.md`.
