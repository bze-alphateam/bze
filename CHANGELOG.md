...
* (app) [#114](https://github.com/bze-alphateam/bze/pull/114) Noble USDC wind-down in the `v8.2.0` upgrade handler: `blocked_ibc_inbound` is set to `{channel-3, uusdc}`, so new Noble USDC can no longer enter the chain, and the burner black hole's entire balance of the USDC.n/BZE pool share denom (`ulp_ibc/6490A7EA…_ubze`) is transferred to `bze1jx4x3kn8mlz2s03zdpf2a38x9gl66llvjqdd55` — protocol-owned liquidity handed to the admin address so it can be redeployed as USDC.inj liquidity and locked again. Every other black-hole balance is untouched; a zero balance is a logged no-op, and a failed transfer is logged instead of failing the upgrade. The testnet (`bzetestnet-3`) runs the same two steps against the assets it has — its own transfer channel and the pool shares its black hole holds — so the upgrade can be rehearsed end to end.
